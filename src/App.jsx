...
function getPeriodStats(tasks, type){
  const now = new Date();
  ...
  const done = tasks.filter(t=>t.done && t.completedAt && new Date(t.completedAt)>=start);
  return {
    count: done.length,
    score: done.reduce((a,t)=>a+calcValueScore(t),0),
    p01: done.filter(t=>t.purpose==="01").length,
    p02: done.filter(t=>t.purpose==="02").length,
    p03: done.filter(t=>t.purpose==="03").length,
  };
}

function AchievementDashboard({tasks=[]}){
  const card=(title,type)=> {
    const s=getPeriodStats(tasks,type);
    return (
      <div style={{background:P.surface,borderRadius:18,padding:"12px 14px",border:`1px solid ${P.border}`,marginBottom:10}}>
        <div style={{fontSize:10,color:P.inkFaint,letterSpacing:".1em",marginBottom:8}}>{title}</div>
        <div style={{fontSize:24,color:P.ink}}>{s.count}<span style={{fontSize:12}}> 件</span></div>
        <div style={{fontSize:12,color:P.fiesta,fontWeight:700}}>{s.score} pt</div>
        <div style={{fontSize:10,color:P.inkSub,marginTop:6}}>01売上 {s.p01}件 / 02改善 {s.p02}件</div>
      </div>
    )
  };
  return <div>
...
/* -------------------------------------- */
      ...
      <div className="col-left left-scroll">
        ...
        {/* chatwork reminder */}
          ...
          {/* weekly top5 */}
          <div style={{background:P.surface,borderRadius:18,padding:"14px 16px",border:`1px solid ${P.border}`,marginBottom:10}}>
            <div style={{fontSize:10,color:P.inkFaint,letterSpacing:".12em",marginBottom:10}}>今週TOP5</div>
            {tasks.filter(t=>!t.done).map(t=>({...t,score:calcValueScore(t)})).sort((a,b)=>b.score-a.score).slice(0,5).map((t,i)=>(
              <div key={t.id} style={{fontSize:12,padding:"6px 0",borderBottom:`1px solid ${P.border}`}}>
                {i+1}. {t.text}<span style={{float:"right",color:P.fiesta}}>{t.score}pt</span>
              </div>
            ))}
          </div>
